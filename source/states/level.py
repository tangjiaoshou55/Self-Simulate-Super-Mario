...
class Level:
    ...
    def setup_enemies(self):
        ...
        self.enemy_group_dict = {} # the dictionary for enemies
    ...
    def check_x_collisions(self):
        ...
        if shell:
            if shell.state == 'slide': # if shell touch Mario at slide state
                self.player.go_die() # Mario dies
            else:
                if self.player.rect.x < shell.rect.x: # If we touch the turtle shell from left to right
                    shell.x_vel = 10 # The tortoise shell flicks to the right
                    shell.rect.x += 40
                    shell.direction = 1
                else:
                    shell.x_vel = -10 # Otherwise, the tortoise shell flicks to the left
                    shell.rect.x -= 40
                    shell.direction = 0
                shell.state = 'slide'
    ...
    def adjust_player_y(self, sprite):
        ...
        # upward
            ...
            self.is_enemy_above(sprite)

            if sprite.name == 'box':
                if sprite.state == 'stay':
                    sprite.go_bumped()
            if sprite.name == 'brick':
                if self.player.big and sprite.brick_type == 0: # when mario is big and bricks contain nothing
                    sprite.smashed(self.dying_group)
                else:
                    sprite.go_bumped()

    def is_enemy_above(self, sprite):
        sprite.rect.y -= 1
    ...
    def update_game_window(self):
        third = self.game_window.x + self.game_window.width / 7 * 3
    ...
    def check_checkpoints(self):
        ...
        if checkpoint:
            ...
            checkpoint.kill() # once touch, destroy the checkpoint
