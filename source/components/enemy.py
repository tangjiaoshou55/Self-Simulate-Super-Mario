...
class Enemy(pygame.sprite.Sprite):
    ...
    def fall(self): # If the falling speed is less than the limit, add a gravity speed.
        if self.y_vel < 10:
    ...
    def update_position(self, level):
        ...
        self.rect.y += self.y_vel
        if self.state != 'die': # Collision detection is only performed when the enemy is not dead
...
class Koopa(Enemy):
    def __init__(self, x, y_bottom, direction, name, color):
        green_frame_rects = [(96, 9, 16, 22), (112, 9, 16, 22), (160, 9, 16, 22)]
        blue_frame_rects = [(96, 72, 16, 22), (112, 72, 16, 22), (160, 72, 16, 22)]

        if not color:
            self.frame_rects = green_frame_rects
        else:
    ...
    def trampled(self, level):
        ...
        if self.current_time - self.shell_timer > 3000: # do not touch shell for 3 second
            self.state = 'walk' # the Koopa comes back in walk state
